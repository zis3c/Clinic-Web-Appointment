        ...
        <Modal show={show} onClose={onClose} maxWidth="md">
            {internalApt && (
                <div className="p-8 relative dark:bg-slate-800">
                    ...
                    <button 
                        onClick={onClose}
                        className="absolute top-6 right-6 text-gray-400 hover:text-rose-500 transition-all duration-300 p-1.5 bg-gray-50 dark:bg-slate-700 hover:bg-rose-50 dark:hover:bg-rose-900/30 rounded-full shadow-sm hover:shadow outline-none focus:outline-none focus:ring-0 active:outline-none"
                    >
                    ...
                    {/* Modal Title */}
                    <div className="mb-6">
                            <span className="text-[10px] font-black text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-blue-900/30 px-2.5 py-1 rounded-md uppercase tracking-wider">
                        ...
                        <div className="p-5 bg-slate-50 dark:bg-slate-900/50 border border-slate-100 dark:border-slate-700 rounded-2xl mb-6">
                            <div className="flex justify-between items-center mb-3">
                                <span className="text-xs text-gray-500 dark:text-gray-400 font-bold uppercase tracking-wider">Current State</span>
                            ...
                            <div className="text-sm font-semibold leading-relaxed border-t border-slate-200/50 dark:border-slate-700/50 pt-3">
                                ...
                                {internalApt.status === 'confirmed' && !internalApt.checked_in && (
                                    <p className="text-blue-700 dark:text-blue-400">Confirmed. Please notify reception upon arrival at the clinic.</p>
                                )}
                                {internalApt.status === 'confirmed' && !!internalApt.checked_in && (
                        ...
                        <div className="space-y-4 mb-8">
                            <div>
                                <label className="block text-[10px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-widest mb-1">Doctor</label>
                            ...
                            <div className="grid grid-cols-2 gap-4 pt-3 border-t border-gray-100 dark:border-slate-700">
                                <div>
                                    <label className="block text-[10px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-widest mb-1">Scheduled Date</label>
                                    <p className="text-sm font-bold text-gray-900 dark:text-slate-100">
                                        {new Date(internalApt.date).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}
                                    </p>
                                </div>
                                <div>
                                    <label className="block text-[10px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-widest mb-1">Session Time</label>
                                    <p className="text-sm font-bold text-gray-900 dark:text-slate-100">
                    ...
                    <div className="flex gap-3">
                        <button 
                            onClick={onClose}
                            className="w-full px-4 py-3 bg-gray-50 dark:bg-slate-700 hover:bg-gray-100 dark:hover:bg-slate-600 border border-gray-200 dark:border-slate-600 text-gray-700 dark:text-gray-200 font-black rounded-xl transition-all duration-300 shadow-sm hover:shadow-md hover:-translate-y-1 active:translate-y-0 text-sm outline-none focus:outline-none focus:ring-0 active:outline-none"
                        >
                            Close
                        </button>
                    </div>
