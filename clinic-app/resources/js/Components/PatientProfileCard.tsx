...
export default function PatientProfileCard({ 
    ...
    return (
        <div className={`flex flex-col h-full relative ${variant === 'split' ? 'p-8' : ''}`}>
            {variant === 'modal' ? (
                <div className="flex items-center justify-between sticky top-0 bg-white/80 dark:bg-slate-900/80 backdrop-blur-md z-10 px-8 py-6 border-b border-gray-100 dark:border-slate-800">
                    <h3 className="text-2xl font-bold text-gray-900 dark:text-white">Patient Details</h3>
                    {onClose && (
                        <button onClick={onClose} className="text-gray-400 dark:text-gray-500 hover:text-rose-500 dark:hover:text-rose-400 transition-all duration-300 bg-gray-50 dark:bg-slate-800 hover:bg-rose-50 dark:hover:bg-rose-900/30 p-1.5 rounded-full shadow-sm outline-none focus:outline-none focus:ring-0 active:outline-none">
                                    ...
                                    <div className="flex flex-row gap-2 w-full">
                                        {patient.allergies && (
                                            <div className="bg-rose-50 dark:bg-rose-900/30 rounded-lg p-2 border border-rose-100 dark:border-rose-800/30 flex-1 min-w-0 flex items-center shadow-sm">
                                                <span className="text-[10px] font-bold uppercase tracking-wider text-rose-700 dark:text-rose-400 mr-2 shrink-0">Allergies:</span>
                                                <span className="text-xs font-bold text-rose-950 dark:text-rose-200 truncate">{patient.allergies}</span>
                                            </div>
                                        )}
                                        {patient.medical_conditions && (
                                            <div className="bg-rose-50 dark:bg-rose-900/30 rounded-lg p-2 border border-rose-100 dark:border-rose-800/30 flex-1 min-w-0 flex items-center shadow-sm">
                                                <span className="text-[10px] font-bold uppercase tracking-wider text-rose-700 dark:text-rose-400 mr-2 shrink-0">Conditions:</span>
