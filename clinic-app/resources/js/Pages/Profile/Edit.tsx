...
import UpdateMedicalProfileForm from './Partials/UpdateMedicalProfileForm';

export default function Edit({ auth, mustVerifyEmail, status, sessions = [], patientData }: any) {
    ...
    if (patientData) {
        tabs.push({ id: 'medical', name: 'Medical Profile', icon: 'M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z' });
    }
    ...
    return (
        ...
        >
            ...
            <div className="h-auto lg:h-[calc(100vh-4rem)] min-h-[600px] w-full flex flex-col md:flex-row bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-gray-200 dark:border-slate-700 overflow-hidden max-w-7xl mx-auto transition-all">
                        ...
                        {tabs.map((tab) => (
                            <button
                                ...
                                className={`w-full flex items-center gap-3 px-4 py-3 text-left rounded-xl transition-all duration-200 font-medium border ${
                                    activeTab === tab.id 
                                    ? 'bg-white dark:bg-slate-800 text-blue-700 dark:text-blue-400 shadow-sm border-gray-200 dark:border-slate-700' 
                ...
                <div className="flex-1 bg-white dark:bg-slate-800 overflow-y-auto transition-all">
                    <div className="p-8 lg:p-12 max-w-3xl">
                        ...
                        {/* Medical Profile Tab */}
                        {activeTab === 'medical' && patientData && (
                            <div className="animate-in fade-in slide-in-from-right-4 duration-300">
                                <h3 className="text-2xl font-bold text-gray-900 dark:text-white mb-2">Medical Profile</h3>
                                <p className="text-sm text-gray-500 dark:text-gray-400 mb-6">Update your medical information so doctors are aware of your conditions and allergies.</p>
                                <UpdateMedicalProfileForm patientData={patientData} />
                            </div>
                        )}
