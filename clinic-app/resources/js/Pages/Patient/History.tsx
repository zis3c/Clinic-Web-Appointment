...
export default function History({ auth, appointments }: any) {
    ...
    const filteredAppointments = appointments?.filter((apt: any) => {
        ...
        const doctorName = apt.schedule?.doctor?.user?.name?.toLowerCase() || '';
        const specialty = apt.schedule?.doctor?.specialty?.name?.toLowerCase() || '';
    ...
    return (
        ...
        >
            ...
            <div className="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
                ...
                {/* Header Actions */}
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-700 transition-all">
                    <div>
                        <h3 className="text-base md:text-lg font-bold text-gray-800 dark:text-slate-100">Your Medical History</h3>
                        <p className="text-xs md:text-sm text-gray-500 dark:text-gray-400">View your past clinic visits, diagnoses, and doctor's prescriptions.</p>
                    </div>
                    <div className="flex flex-col sm:flex-row gap-3 w-full md:w-auto mt-4 md:mt-0">
                        <a 
                            href={route('patient.history.export')} 
                            target="_blank"
                            title="Download History"
                            className="flex items-center justify-center w-9 h-9 sm:w-11 sm:h-11 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white rounded-xl shadow-md transition-all duration-300 shrink-0"
                        >
                            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"></path></svg>
                        </a>
                        <div className="flex-1 sm:flex-none sm:w-44 z-20 relative h-9 sm:h-11">
                            <CustomSelect
                                value={sortOrder}
                        ...
                        <div className="relative flex-1 sm:flex-none">
                            <div className="absolute top-1/2 -translate-y-1/2 left-3.5 flex items-center pointer-events-none">
                ...
                {sortedAppointments && sortedAppointments.length > 0 ? (
                    <div className="relative border-l-[3px] border-blue-500/30 dark:border-blue-500/20 ml-5 md:ml-6 py-2">
                        {sortedAppointments.map((apt: any, _index: number) => {
                                        ...
                                        <div className="flex flex-col md:flex-row md:items-start justify-between gap-4 mb-5">
                                            <div>
                                                ...
                                                <h3 className="text-base md:text-lg font-bold text-gray-900 dark:text-white flex items-center gap-2">
                                                    Consultation with Dr. {apt.schedule?.doctor?.user?.name || 'Unknown'}
                                                </h3>
                                                <p className="text-gray-500 dark:text-gray-400 text-xs mt-0.5">{apt.schedule?.doctor?.specialty?.name}</p>
                                            </div>
                                            <div className="flex items-center gap-2 self-start md:self-auto">
                                                ...
                                                <a 
                                                    ...
                                                    className="w-7 h-7 bg-blue-500 hover:bg-blue-600 text-white rounded-xl flex items-center justify-center transition-all shadow-sm outline-none focus:ring-2 focus:ring-blue-500/40"
                                                >
                    ...
                    <div className="bg-white dark:bg-slate-800 rounded-2xl p-10 text-center border border-gray-100 dark:border-slate-700 shadow-sm mt-6">
                        ...
                        <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-2">No Medical History Found</h3>
                        <p className="text-gray-500 dark:text-gray-400 max-w-sm mx-auto text-sm leading-relaxed">
                            {searchQuery ? "No past visits match your search." : "You haven't had any completed consultations yet."}
                        </p>
                    </div>
                )}
            </div>
