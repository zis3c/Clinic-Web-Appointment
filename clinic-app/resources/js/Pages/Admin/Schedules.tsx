                        ...
                        <table className="min-w-full divide-y divide-gray-200 dark:divide-slate-700">
                            <thead className="bg-gray-50/80 dark:bg-slate-800/80 backdrop-blur-md sticky top-0 z-10 ring-1 ring-gray-200 dark:ring-slate-700">
                                <tr>
                                    ...
                                    <th className="px-6 py-4 text-left text-xs font-bold text-gray-500 dark:text-gray-400 capitalize tracking-wider">Title / Doctor</th>
                                    ...
                                    <th className="px-6 py-4 text-left text-xs font-bold text-gray-500 dark:text-gray-400 capitalize tracking-wider">Bookings</th>
                            ...
                            <tbody className="bg-white dark:bg-slate-800 divide-y divide-gray-200 dark:divide-slate-700">
                                {filteredSchedules.map((schedule: any, index: number) => (
                                    <tr
                                        ...
                                        className={`transition-all cursor-pointer ${selectedIds.includes(schedule.id) ? 'bg-blue-50/50 dark:bg-blue-900/20' : 'hover:bg-blue-50 dark:hover:bg-slate-700/50'}`}
                                    >
                                        {isBulkMode && (
                    ...
                    {filteredSchedules.length === 0 ? (
                        ...
                        filteredSchedules.map((schedule: any) => (
                            <div 
                                ...
                                className={`bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-700 p-4 flex flex-col gap-3 cursor-pointer transition-all ${selectedIds.includes(schedule.id) ? 'bg-blue-50/70 dark:bg-blue-900/20' : 'hover:bg-gray-50 dark:hover:bg-slate-700/50'}`}
                            >
                                    ...
                                    <div className="flex items-center gap-3">
                                        {isBulkMode && (
                                            <input 
                                                type="checkbox" 
                                                checked={selectedIds.includes(schedule.id)}
                                                onChange={(e) => { e.stopPropagation(); handleSelect(schedule.id); }}
                                                className="w-5 h-5 rounded border-gray-300 dark:border-slate-700 text-blue-600 focus:ring-blue-500 cursor-pointer"
                                            />
                                        )}
                                        <div>
                                            <h4 className="font-bold text-gray-900 dark:text-slate-100">{schedule.title}</h4>
                                            <p className="text-sm font-semibold text-blue-600 dark:text-blue-400 mt-0.5">Dr. {schedule.doctor.user.name}</p>
                                        </div>
                                    </div>
                                    {!isBulkMode && (
                                        <button 
                                            onClick={(e) => { e.stopPropagation(); setSessionToDelete(schedule); setIsCancelModalOpen(true); }}
                                            className="w-8 h-8 flex items-center justify-center rounded-lg bg-rose-50 dark:bg-rose-900/30 text-rose-600 dark:text-rose-400 hover:bg-rose-100 dark:hover:bg-rose-900/50 transition-all"
                                        >
                                            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg>
                                        </button>
                                    )}
                                ...
                                <div className="flex justify-between items-center text-sm pt-2 border-t border-gray-100 dark:border-slate-700">
                                    <span className="text-gray-500 dark:text-gray-400">{new Date(schedule.date).toLocaleDateString()} • {schedule.time}</span>
                                        ...
                                        <form onSubmit={submit} className="space-y-4">
                                            ...
                                            <div className="relative z-50">
                                                ...
                                                <CustomMultiSelect
                                                    ...
                                                    className="w-full px-4 py-3 rounded-xl border border-gray-200 dark:border-slate-700 bg-gray-50 dark:bg-slate-900/50 dark:placeholder-gray-500 placeholder:font-normal text-gray-700 dark:text-slate-200 transition-all outline-none shadow-sm font-medium cursor-pointer"
                                                />
                ...
                {viewSchedule && (
                    <div className="p-4 sm:p-6 md:p-8 dark:bg-slate-800">
                        <div className="flex items-center justify-between mb-6">
                            ...
                            <button onClick={() => setShowScheduleModal(false)} className="text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 bg-gray-50 dark:bg-slate-700 hover:bg-gray-100 dark:hover:bg-slate-600 rounded-full p-2 transition-all">
                                ...
                                <div className="col-span-2">
                                    ...
                                    {viewSchedule.doctor?.tel ? (
                                        <a
                                            ...
                                            className="inline-flex items-center font-bold text-gray-900 dark:text-slate-100 hover:text-emerald-600 transition-all group"
