                            ...
                            <tbody className="bg-white divide-y divide-gray-200">
                                {filteredSchedules.map((schedule: any) => (
                                    <tr 
                                        ...
                                        className={`transition-colors cursor-pointer ${selectedIds.includes(schedule.id) ? 'bg-blue-50/50' : 'hover:bg-blue-50'}`}
                                    >
                                        {isBulkMode && (
                ...
                {viewSchedule && (
                    <div className="p-8">
                        <div className="flex items-center justify-between mb-6">
                            ...
                            <button onClick={() => setShowScheduleModal(false)} className="text-gray-400 hover:text-gray-600 bg-gray-50 hover:bg-gray-100 rounded-full p-2 transition-colors">
                                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M6 18L18 6M6 6l12 12"></path></svg>
                            </button>
                        </div>
                        <div className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm space-y-4">
                            <div className="border-b border-gray-100 pb-4 mb-4">
                                <h4 className="text-xl font-bold text-gray-900">{viewSchedule.title}</h4>
                                <p className="text-sm text-blue-600 font-semibold mt-1">Dr. {viewSchedule.doctor?.user?.name || 'Unknown'}</p>
                            </div>
                            <div className="grid grid-cols-2 gap-4 text-sm">
                                <div>
                                    <p className="text-gray-500 font-medium mb-1">Date</p>
                                    <p className="font-bold text-gray-900">{new Date(viewSchedule.date).toLocaleDateString()}</p>
                                </div>
                                <div>
                                    <p className="text-gray-500 font-medium mb-1">Time</p>
                                    <p className="font-bold text-gray-900">{viewSchedule.time}</p>
                                </div>
                                <div>
                                    <p className="text-gray-500 font-medium mb-1">Capacity</p>
                                    <p className="font-bold text-gray-900">{viewSchedule.number_of_patients} patients</p>
                                </div>
                                <div>
                                    <p className="text-gray-500 font-medium mb-1">Booked</p>
                                    <span className="px-3 py-1 inline-flex text-xs leading-5 font-semibold rounded-full bg-blue-100 text-blue-800">
                                        {viewSchedule.appointments?.length || 0} / {viewSchedule.number_of_patients}
                                    </span>
                                </div>
                                <div className="col-span-2">
                                    <p className="text-gray-500 font-medium mb-1">Doctor Contact</p>
